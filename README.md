...
本次学习主要是了解了内核启动的流程，更清楚的明白从BIOS到内核初始化的整个过程。

首先BIOS会载入512字节的扇区到内存中，并跳转执行最基础的指令进行初始拷贝操作，其将内核elf数据载入到内存中，之后再去跳转内核初始化函数从而进行真正的内核初始化。这期间涉及到了实模式到保护模式的转换，以及页表模式的启动和虚拟地址到物理地址的映射流程。

总而言之，此次学习收获颇多，为后续的操作系统学习奠定了一定基础。